...
export async function zohoApiRequest(
	...
	const { oauthTokenData } = (await this.getCredentials(
		'zohoDeskOAuth2Api',
	)) as ZohoDeskOAuth2ApiCredentials;

	const options: OptionsWithUri = {
		body: body,
		method,
		qs,
		uri: `${uri}/api/v1${endpoint}` || `${oauthTokenData.api_domain}/api/v1${endpoint}`,
		json: true,
	};
	...
	try {
		const responseData = await this.helpers.requestOAuth2?.call(this, 'zohoDeskOAuth2Api', options);
